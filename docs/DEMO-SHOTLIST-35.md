# Shot list — five minutes

Recording target: **1440×900**, browser chrome hidden, `./run-demo` already up.
Cursor moves only where a line below says it does. No wandering, no hovering to
fill silence.

Every row's "number visible" must be legible in frame when the line is spoken.
If it is not on screen, cut the line — do not speak it from memory.
...
| 2:28 | hold on the counterfactual | none | — | "27 of 63 · 42.9%" | 27 of 63 · 42.9% |
...
| 2:45 | scroll to §06 the machinery | scroll | — | four properties | 28 · 6 of 6 · 42.9% · 21 of 21 |
| 3:00 | hold on column 1 | none | — | "checker does not import the solver" | 28 LINE KERNEL |
| 3:10 | hold on column 2 | none | — | integer paise, 34 attempts, 0 succeeded | 6 OF 6 GATES |
...
| 4:05 | hold | none | — | "no button, deliberately" | Supply an order-level reference. |
...
## Optional 20-second insert (only if the take runs short)

Click **OPEN THE INVESTIGATION**, land on `/app` at `setl_000225` in Evidence,
say: *"the same case, in the instrument — same verdict, same canvas."* Then cut
back. Do not tour the seven instruments; it costs a minute and proves nothing
the front door has not already shown.
...
- Explaining the seven instruments.
- Apologising for the data being synthetic. It is a design decision with a
  stated reason; say the reason once and move on.
